...
bin/playtest-routes Dungeon cut d-2 --from-commands d2.txt     # a new one, by hand
bin/playtest-routes Dungeon distill d-2 --from-session <probe> --upto 44
                                               # a new one, learned from a round's own
                                               # play: drop what cost no turn, then drop
                                               # contiguous runs and replay after each,
                                               # keeping a cut only if it still lands
...
- **A flag that only ever goes up is `@Latch`, not `@Global`.** `@Latch var crawlBeatDone` takes no initial value, reads as a plain `Bool` anywhere (a rule, a `describe { }`, a conditional exit), and has **no setter at all** — `= false` and `= true` both fail to compile. `$crawlBeatDone.trips()` is the only way it changes and returns whether *this* call raised it, so `guard !flag else { … }` + `flag = true` collapses to `guard $flag.trips() else { … }` and the two halves cannot come apart. Underneath it is a `@Global`: same `WorldState.globals` storage, same bundle namespacing, same commit, UNDO, save and restore, and no bootstrap registration of its own. The check is a **compile error rather than a bootstrap warning**, deliberately — the bootstrap never sees a rule body, so it cannot know a flag is read once and set once. Two traps: `trips()` raises the latch as it answers, so in a multi-condition guard it must come **last** (`guard $baubleDropped.trips(), inTheForest else { … }` spends the bauble on a turn the player was never in the forest); and a flag genuinely written both ways — a door that opens *and* shuts — is not a latch, so leave it a `@Global`.
...
- **UNDO, RESTART, SAVE, RESTORE, AGAIN, OOPS, VERBOSE, BRIEF and SUPERBRIEF can't be overridden at all.** The engine answers them before the pipeline, so no rule sees them and `action(.save)` never runs. That's `DefaultActions.engineIntents`, and declaring one now warns rather than failing silently. AGAIN and OOPS hand a *different line* back to the parser rather than acting themselves: `again`/`g` re-parses `WorldState.lastCommand` and costs whatever that costs, and `oops <word>` rewrites the word the parser last refused. Neither is recorded as a last command, which is what makes AGAIN non-recursive by construction rather than by a depth counter.
