...
- **A listing line is not an examine line, and nine items had been given the same sentence on both channels.** The burned-out lantern, the rusty knife, the jade figurine, the huge diamond, the folded pile of plastic, the red buoy, the pot of gold, the platinum bar and the bird's nest each carry one string in the source — an `FDESC` or an `LDESC`, both of them *listing* properties — and none of the nine has a `TEXT`. So the original answers `EXAMINE` on each of them with `V-EXAMINE`'s stock *"There's nothing special about the X."* (`gverbs.zil:623`), apart from the red buoy and the bird's nest: both carry `CONTBIT`, and `V-EXAMINE` looks inside them instead, which the port does not do (see the container entry below). The listing line belongs on the listing channel, so each item declares `firstSight(…)` and no `description(…)`. Wiring the listing line to both channels made `x pot of gold` answer *"At the end of the rainbow is a pot of gold."* while the pot was in the player's hands. The guard is wherever each item's own region suite walks past it — the rusty knife and the burned-out lantern in `Zork1MazeTests`, and so on. (#350; this entry previously named a `ListingLineChannelTests` suite that has never existed, which is how the seven in #514 went unnoticed.)
...
- **`EXAMINE` does not look inside a container.** `V-EXAMINE` (`gverbs.zil:623`) prints an object's `TEXT` if it has one, and otherwise sends an object with `CONTBIT` or `DOORBIT` to `V-LOOK-INSIDE`, which says the container is closed, says it is empty, or lists what it holds. The engine's `EXAMINE` has no such branch, so the mailbox, the brown sack, the glass bottle, the red buoy and the bird's nest answer *"There's nothing special about the …"*, where the original says *"The small mailbox is closed."*, *"The brown sack is closed."* and *"The red buoy is closed."* while they are shut, and lists the water in the bottle and the egg in the nest. `look in` gives the engine's own answer to the same question. Not reproduced: a `describe { }` on each container could restate `V-LOOK-INSIDE`, but it would be a second contents list beside the engine's, in a different format. (#618)
...
- **The grue rolls the dice** *(closed in the fidelity pass — was a deterministic linger clock)*, like the original: a warning on the first consecutive turn *ending* in darkness (the kept fairness beat — the grue never eats you on the turn the dark begins), one silent grace turn, then from the third dark turn on it rolls `chance(lethality)` (Zork 1 uses 50%) to be eaten each turn; any lit turn resets the count. The daemon lives in the `GnustoDangerousDark` plugin (`graceTurns` and `lethality` are knobs); Zork 1 passes its own prose in. The warning is `DESCRIBE-ROOM`'s dark line, *"It is pitch black. You are likely to be eaten by a grue."* (`gverbs.zil:1637-1640`), which Zork 1 also sets as `text.pitchBlack`, and the death line is `GOTO`'s (see "The grue has two deaths" below). Because death is a roll, the dark-lingering transcripts are seed-pinned (the cost of the dice). **UNDO after a grue death does not restore the warning.** The dark-turn count is a `@Global`, so UNDO puts it back one turn and the next dark turn rolls again; the seeded random stream is rewound as well, so the same command rolls the same death. The sentence printed after that UNDO reads like the warning because it is the room's `pitchBlack` line. (#628)
...
- **Room, item, and message prose is held to the Zork 1 rule at the top of this file**, one constant per entity in `Prose+RoundRoom.swift`, as in every prior task. The scenery examine texts #407 added there are the port's own; see Task 8's *Prose* entries. Room and treasure *names* ("Round Room", "Loud Room", "platinum bar") are the iconic ones, used as-is.
...
- **Room, item, and message prose is held to the Zork 1 rule at the top of this file**, one constant per entity in `Prose+Dam.swift`, as in every prior task. The scenery examine texts #407 added there are the port's own; see Task 8's *Prose* entries. Room and item *names* ("Dam", "Maintenance Room", "trunk of jewels", "hand-held air pump") are the iconic ones, used as-is.
...
**This section is about both halves of the ledger.** The three description-mode verbs are the engine's, not either game's. Dungeon declares nothing for them and answers with the engine's stock lines throughout. Zork 1 declares one of the three, below, to restore the original's spelling; the other two are the engine's stock lines verbatim in both games. Before #499 the engine had no such verb and `verbose` in either game answered *I don't know the word "verbose"*, which is why the departure is worth writing down.

The verbs and the precedence are the original's. `V-VERBOSE` and `V-BRIEF` in `gverbs.zil` answer *"Maximum verbosity."* and *"Brief descriptions."*, and those two are the engine's stock lines verbatim. `V-SUPER-BRIEF` answers *"Super-brief descriptions."* with a hyphen, where the engine's stock line follows the modern ZIL library and writes the word closed up — so `Sources/Zork1/` sets `text.superbriefDescriptions` back to the hyphenated spelling and Dungeon, which adapts, keeps the stock line. `gsyntax.zil` declares `SUPER` as a synonym of `SUPERBRIEF`, and the engine takes `super` as well. `DESCRIBE-ROOM` prints the long description when the line was an explicit LOOK or `VERBOSE` is set, which is the precedence the engine reproduces; `gmain.zil` lists all three among the verbs that skip `CLOCKER`, the original's timer tick, and they are meta intents here for the same reason.

**One departure beyond that spelling.** `V-FIRST-LOOK` in `gverbs.zil` calls `DESCRIBE-OBJECTS` only when `SUPER-BRIEF` is unset, so the original's superbrief entry prints the room's name and nothing else — not even the things lying on the floor. The engine's superbrief withholds the long description and keeps the item paragraphs, so an entry reads as a name plus what is there to pick up. That is the owner's decision on #499, taken as a usability call rather than as a reading of the source: a mode that hides the objects hides the game. The two games inherit it because the engine owns the verb.
...
- **What the player drops inside the Cage is on the closet floor after the robot lifts it.** In the source the sphere is never inside the cage. It has no `TAKEBIT` (`dung.355:4694`), and when the player takes it while `CAGE-SOLVE!-FLAG` is false, `SPHERE-FUNCTION` (`act3.199:231`) moves the player and the robot into `CAGED` and does not move the sphere, which stays in the closet. The Fortran does the same (`objects.f:404`). This game lets that take complete: the player is in the Cage holding the sphere, the six points for finding it are paid on the take, and the robot stays in the closet. That is a departure of its own, and it is why a sphere can be dropped inside the Cage here. `ROBOT-ACTOR` (`act3.199:268`) answers `robot, lift the cage` by moving the player, the cage object and the robot to `CAGER` and giving the sphere its `TAKEBIT`, and `timefnc.f:968` does the same. It moves nothing else out of `CAGED`, and `CAGED` has no exit, so in the source anything else the player set down inside the cage stays there. The lift rule in `Sources/Dungeon/Regions/Alice.swift` moves every takable item on the Cage's floor to the Dingy Closet before it moves the player, because the cage was standing on the closet floor. (#620)
...
### The canaries' examine texts (#617)

The mainframe's `ODESCO` for each canary and Zork I's `FDESC` for it are the same paragraph, word for word once the mainframe's doubled spaces are closed up, and it opens *"There is a golden clockwork canary nestled in the egg."* Both birds had that paragraph as their `description(…)`, so EXAMINE said the bird was in the egg while it was in the player's hand.

- **Both examine texts are the paragraph with its opening clause repaired and the rest verbatim** — *"A golden clockwork canary. It has ruby eyes and a silver beak…"* — the way `Prose.egg` already repairs the egg's.
- **The ruined bird's paragraph is its listing line.** `BCANA` starts offstage inside `BEGG`, so the line prints wherever the wreck stands in a room with the bird untouched inside it.
- **The intact bird has no listing line of its own**, for the reason Zork 1's #617 entry gives: `GCANA` starts inside the egg on the nest, where the bootstrap warns about a listing line. Where the thief's opened egg stands in a room, the canary in it gets the engine's stock sentence, as it did before.
