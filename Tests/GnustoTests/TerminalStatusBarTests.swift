...
struct TerminalStatusBarTests {
    /// A status line for the bar to paint. The id is the same throwaway every
    /// time: the bar never reads it, and these tests are about columns.
    private func line(_ name: String, score: Int, moves: Int) -> StatusLine {
        StatusLine(locationID: EntityID("room"), locationName: name, score: score, moves: moves)
    }

    @Test("Pads to the full width with the score flush right")
    func padsAsciiToWidth() {
        let bar = TerminalIOHandler.statusBar(
            line("West of House", score: 0, moves: 0), cols: 40)
    ...
    func padsWideNameToWidth() {
        ...
        let bar = TerminalIOHandler.statusBar(
            line("西の家", score: 10, moves: 3), cols: 40)
        #expect(DisplayWidth.columns(of: bar) == 40)
    ...
    func clipsByColumn() {
        let bar = TerminalIOHandler.statusBar(
            line("A Very Long Room Name Indeed", score: 999, moves: 999), cols: 20)
    ...
    func clipsWideNameByColumn() {
        let bar = TerminalIOHandler.statusBar(
            line("西の家西の家西の家西の家", score: 999, moves: 999), cols: 10)
