...
struct DungeonEndgame: GameContent {
    ...
    let tombFloor = Item.scenery("tomb floor", synonyms: "floor", "ground", description: Prose.tombFloor)
    ...
    /// sentences on. (#332)
    let stoneRoomMasonry = Item.scenery(
    ...
    let topOfStairsLanding = Item.scenery(
    ...
    let redBeam = Item.scenery("red beam of light", adjectives: "red", "thin", synonyms: "beam", "light", "ray")

    /// `RSWIT`. Opens the mirror for seven turns, but only with the beam
    /// broken; otherwise it pops straight back out.
    let redButton = Item.scenery(
    ...
    let pineEnd = Item.scenery("pine wall", adjectives: "pine", "pale", synonyms: "pine", "wall", "end", "door")
    ...
    private static func mirrorPanel(_ ordinal: String) -> Item {
        Item.scenery("\(ordinal) mirror", adjectives: ordinal, "large", synonyms: "mirror", "mirrors", "glass")
    }
    ...
    private static func panel(_ colour: String) -> Item {
        Item.scenery("\(colour) panel", adjectives: colour, synonyms: "panel", "panels")
    }
    ...
    /// reads it.
    let pole = Item.scenery("pole", adjectives: "long", "short", "wooden", synonyms: "pole", "rod", "shaft")

    /// `TBAR`. The handle the pole is raised and lowered by.
    let tBar = Item.scenery(
    ...
    let lockedCellDoor = Item.scenery(
        "locked door",
    ...
    private static func slot() -> Item {
        Item.scenery("doorway", adjectives: "cut", "stone", synonyms: "doorway", "slot", "opening", "shaft")
    }
    ...
    let sundial = Item.scenery("sundial", adjectives: "stone", "sun", synonyms: "dial", "sundial", "pointer")
    ...
    private static func numeral(_ number: Int) -> Item {
        ...
        Item.scenery(numberWord(number), synonyms: "\(number)", description: Prose.sundialNumeral)
    }
    ...
    let parapetButton = Item.scenery(
