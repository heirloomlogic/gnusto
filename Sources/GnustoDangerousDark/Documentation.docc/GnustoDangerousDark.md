...
**The death this daemon deals is the lingering one**, and the line has to be true of that. The first dark turn only warns, and the dice wait for dark turn `graceTurns + 2` — the third dark turn at the default grace of one. So a death here lands on a turn spent lingering in a dark the player has already been warned about, rather than on the turn they walked into it. Zork draws the distinction with two separate sentences and this is the second of them: `V-WALK` (`gverbs.zil:1578`) says *"You have walked into the slavering fangs of a lurking grue!"* and fires only on a **blocked** move in the dark, which is a branch this library does not have; `GOTO` (`:2110-2114`) names the vehicle you are sitting in, or says "room". Both halves are the game's words — the stock line names no place at all, because a library that has not seen the game should not decide what to call the place somebody was taken from.
