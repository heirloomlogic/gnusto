...
enum Bootstrap {
    ...
    static func buildCore(_ game: some Game) throws -> (GameDefinition, WorldState) {
        ...
        for (id, item) in sortedItems {
            ...
            // nothing asks for. A blank article never reaches this loop: it is
            // a fatal diagnostic above (Gate 1, line ~666), which throws
            // before trait warnings are collected — so `article` here is
            // never blank and needs no re-check.
            if let article = item.article {
                let kind = item.isActor ? "actor" : "item"
                if item.isProperName {
                    traitWarnings.append(
                        "\(kind) \"\(id)\" declares article(\"\(article)\") and properName; "
                            + "a proper name takes no article, so the trait has no effect.")
                } else if item.isPlural {
                    traitWarnings.append(
                        "\(kind) \"\(id)\" declares article(\"\(article)\") and plural; "
        ...
        for (name, declarations) in namesByOwner.sorted(by: { $0.key < $1.key }) {
            ...
            for (owner, event) in declarations {
                ...
                // A running fuse's count rides in every save, and a restore
                // refuses a count past `WorldState.counterLimit`. A declared
                // count that large is either carried into a save no restore
                // will accept, or overridden at every start and never used at
                // all. Neither is what the author meant, so the declaration is
                // refused here, as `after: 0` is. Left to run, the first case
                // surfaces as a bare "Restore failed." on a file the game
                // wrote itself, naming neither the fuse nor the bound.
                if case .fuse(let turns) = event.kind, turns > WorldState.counterLimit {
                    ruleDiagnostics.append(
                        "fuse \"\(name)\" declares after: \(turns); a running fuse's count is "
