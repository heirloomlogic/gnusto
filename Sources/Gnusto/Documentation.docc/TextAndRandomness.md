...
A line about **two** things is a `Line` too, over a role struct that names them — ``GameText/Holding`` (a thing and what holds it), ``GameText/Gift``, ``GameText/InstrumentUse``, ``GameText/RestingPlace`` (a thing and whether the player said ON or IN), ``GameText/Aboard``:
