...
It also has a second bundle for the other reason to want one. `Fixtures` owns no geography at all: it is a dozen scenery items that exist so every noun the rooms print is a noun the parser knows, kept out of the host so the host stays short. Its items are placed by the *host's* `map` (`fixtures.sea.starts(in: jetty)`), because a bundle's own `map` can only reach rooms the bundle declares. A bundle is somewhere to put declarations, and "a region" is only the most obvious reason to need one.
