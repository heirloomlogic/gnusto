...
| `the game stores "attic" (Attic) but the game's content block yields a different Attic instance; each Location, Item, Actor and @Global mints its reference token when it is constructed, so the tokens registered are the fresh instance's and every map entry and rule written against attic resolves to nothing. Yield the stored property — 'var content { attic }' — instead of constructing a new one.` | ``Game/content`` said `Attic()` where it meant `attic`. The namespace matches, so every entity ID exists; the tokens do not. |
