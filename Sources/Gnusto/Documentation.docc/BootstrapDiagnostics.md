...
|---|---|
| `content bundles A and B share the namespace "N", so every property name any two of them have in common mints one entity ID and only one of those declarations survives; override 'var namespace' to give each bundle its own.` | Two instances of one ``GameContent`` type, or two types that overrode `namespace` to the same string. Override ``GameContent/namespace`` on one. A type that declares nothing has no identity to compare, so two listings of *it* report the undecidable line below instead. |
| `the game stores "attic" (Attic), a content bundle the game never lists in its content block; nothing it declares — rooms, items, globals, rules, verbs, timers — is registered. Add attic to 'var content'.` | A bundle held as a property but missing from ``Game/content``. A bundle stored by another bundle reads `the content bundle Outer stores "outer.buried" (Buried)…`: listing the holder does not list what it holds, so add the nested bundle under the path that reaches it. |
| `the game stores "attic" (Attic) but the game's content block yields a different Attic instance; each Location, Item, Actor and @Global mints its reference token when it is constructed, so the tokens registered are the other instance's and every map entry and rule written against attic resolves to nothing. If the two were meant to be one bundle, yield the stored property — 'var content { attic }' — instead of constructing a new one. If the game means to have two Attic bundles, list this one too and override 'var namespace' on it, since two Attic bundles under one namespace collide.` | ``Game/content`` said `Attic()` where it meant `attic`, or the game holds two Attic instances and listed one. The namespace matches, so every entity ID exists; the tokens do not. Both readings are stated because the bootstrap cannot tell them apart. |
| `content lists one and the same Attic instance 2 times; a bundle is registered once however often it is named. Remove the extra listing.` | One stored instance yielded twice from ``Game/content``. Overriding ``GameContent/namespace`` is not the cure — there is only one bundle. |
| `content lists 2 Weather bundles under the namespace "Weather", and Weather declares no stored location, item, actor or global, so the bootstrap cannot tell one instance listed 2 times from 2 instances. If it is one, remove the extra listing; if they are separate instances, override 'var namespace' to give each its own.` | A bundle's identity is the reference tokens its declarations mint, and a bundle that declares none has no identity to compare. Both readings are stated because neither can be ruled out. |
...
| `item "box" declares capacity(-1); a capacity cannot be negative. Use 0 to permit nothing, or omit the trait for no limit.` | Also `surfaceCapacity(…)`. Zero is a real declaration — a holder that accepts nothing — and an omitted trait means no limit, so nothing below zero is left to mean. |
...
| `noise word "some" is also an item word; stripping it would make that word untypeable.` | The clause names what it collided with: `a verb word`, `a structural word in a verb pattern`, `a direction`, `a reserved parser word (a pronoun or a multi-object keyword)`, `a word that joins two object phrases`, `a word that excepts objects from a group`, `a possessive the parser drops in front of a noun`, or `an item word`. The four in the middle are the words the parser claims for itself, so filler that lands on one of them costs the word itself: a pronoun, a multi-object keyword, the conjunction, an exception keyword, or a possessive a noun phrase may open with; `her` is in both the reserved set and the possessives and reports as the pronoun. Filler is dropped at tokenize time, before any matching, so a word that is both filler and a real word is a word nobody can type. The built-in filler (`the`, `a`, `an`, `my`, `that`, `this`, `some`, `please`) is checked against your declarations too, which is how an item that answers to `some` gets caught. |
...
| `a before rule (watching take, open) is attached to an item the bootstrap never registered; it must be a stored property of the game, or of a content bundle the game both stores and lists in 'var content'.` | Also `… to a location …`. The rule's scope token is opaque, so the phase and the intents it watches are the anchor for finding it in your source. |
...
| `custom verb "north" can never match: the parser reads a line holding nothing but "north" as the direction, before any verb row is considered.` | A row whose whole pattern is one direction word. A bare direction is answered with ``Intent/go`` ahead of candidate selection, so no table lookup ever reaches the row — which is why the override warning above, keyed off the core verb table, cannot see it. Only that shape: `["north", .directObject]` still matches `north sword`, because a line of two tokens never takes the shortcut. |
...
| `item "shelf" declares surfaceCapacity but is not a surface; the trait has no effect.` | Add ``surface``, or remove the surface capacity. |
...
| `item "sword" declares article("an") and properName; a proper name takes no article, so the trait has no effect.` | Also `… declares article("a") and plural; a plural name takes "some", so the trait has no effect.` And `actor "…"` in place of `item "…"` when the entity is an actor. The indefinite article answers in the order `properName`, `plural`, ``article(_:)``, first letter, so a declared article under either of the first two can never print. Remove it, or remove the trait above it. |
...
| `custom action for intent "undo" will never run; the engine answers undo before the turn pipeline.` | UNDO, RESTART, SAVE, RESTORE, AGAIN, OOPS, VERBOSE, BRIEF and SUPERBRIEF are answered before any stage runs. Nothing can override them. |
...
| `item "brazier" declares alwaysListed but has no firstSight(…) trait and no presence { … } rule; the flag has nothing to keep.` | The item-side twin of the row above. The flag keeps a listing paragraph printing past the first touch, so an item with no listing paragraph reads identically with the flag and without it. Actors are left out of this check: the trait keeps nothing on one either way, and the actor row above already says so, so a declaration earns one sentence rather than two. |
...
It is a flag in the manner of `GNUSTO_PLAIN`, so any value counts, including an empty one. Deliberately not a warning: stack usage varies with build mode, platform and address-space layout, and a machine-dependent figure in the list above would fire on some machines and not others. Dungeon is the largest game in this repository, which is the sense of scale to read its figure against. See <doc:SplittingAGameAcrossFiles>.
