...
| `item "coin" declares firstSight(when: \.isTouched, …) but is not alwaysListed; the listing stops at the first touch, so one of its two texts never prints.` | The listing channel's own dead branches: `\.isTouched` without ``alwaysListed``, `\.isHeld` (`is never listed while it is held`), and on either channel `\.isVisible` (`is only described while it is visible`). The same Bools are live on the examine channel, which is asked whatever the player holds. |
