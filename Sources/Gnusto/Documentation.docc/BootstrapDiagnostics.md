...
| Diagnostic | Cause and fix |
|---|---|
| `item "chest" declares both a static description(…) and a describe { … } rule; an item may have only one.` | Also `location`, and `firstSight(…)` against `presence`. `description(when:_:otherwise:)` and `firstSight(when:_:otherwise:)` count as the static trait. Pick the trait or the rule. |
| `item "chest" declares both description(…) and description(when:_:otherwise:); an item may have only one.` | Also `location`, and `firstSight(…)` against `firstSight(when:_:otherwise:)`. The two-state trait *is* the description; there is no fixed text for it to fall back on. |
