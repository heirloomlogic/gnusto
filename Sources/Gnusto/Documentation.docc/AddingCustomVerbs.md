...
Declaring the property and leaving it empty means what omitting it means — this type declares no verbs — and there are two spellings. An empty body, `var verbs: [SyntaxRule] {}`, works on every builder-backed property, `rules`, `map` and `content` included. An empty array, `var verbs: [SyntaxRule] { [] }`, works on the three that are arrays — `verbs`, `actions`, `timers` — and not on `rules`, `map` or `content`, whose builders collect into a wrapper type rather than an array. Both compile for every property the conforming type actually declares: `verbs`, `actions`, `timers` and `rules` on a `Game`, a ``GameContent`` or a ``GamePlugin`` alike, `map` on a `Game` or a ``GameContent`` (a ``GamePlugin`` declares no `map`), and `content` on a `Game` alone. Write one when a bundle you are stubbing out should say so at the declaration rather than by omission.
...
This is not shorthand for `action(.ring) { try reply(…) }`, and the difference is worth knowing before you reach for the closure. A line takes the path a stub verb takes: it renders the object's name, agrees with its number, and answers `ring me` and `ring the porter` in the engine's own words, none of which a closure does. What the two share is the reach guard. A custom intent declares no ``Reach`` column anywhere else, so both forms take one — `action(.ring, reach: .directObject) { … }` reads the same as `action(.ring, reach: .directObject, say: …)` — and without it `ring the bell` through the glass of a shut case answers as though you were holding it. It defaults to ``Reach/notNeeded``, which is what a custom intent had before the column existed, so adopting either spelling never tightens a verb behind your back — say `reach: .directObject` where the verb has to touch what it names.
