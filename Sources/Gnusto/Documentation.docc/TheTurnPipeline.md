...
0. **Item `reach`** — ``Item/reach(otherwise:_:)`` rules for the command's objects, before any other rules.
1. **World `before`** — ``World/beforeEachTurn(perform:)`` and ``World/before(_:perform:)`` rules.
2. **Location `before`** — ``Location/beforeEachTurn(perform:)`` and ``Location/before(_:perform:)`` rules for the current room.
