...
5. **`after` rules** — ``Item/after(_:perform:)`` rules for the direct object then indirect object, followed by ``Location/after(_:perform:)`` rules for the current room.
6. **End-of-turn rules and the timer tick** — ``Location/afterEachTurn(perform:)``, ``World/afterEachTurn(perform:)``, and ``World/after(_:perform:)`` rules, then every running fuse (firing at zero) and daemon, fuses first and each group in name order. This runs once per typed command, never on parse errors, and not once the game has ended. See <doc:DarknessTimeAndDeath>. Characters take their turns here too: there is no separate actor phase — a roaming thief or a counter-attacking troll is a daemon on this same clock (<doc:ActorsAndVehicles>), which is why your swing resolves in stage 4 and the villain's answer lands at the end of the turn.
