...
The engine reads the player's room once, after stage 1 and before stage 2, and the rule is that simple: **everything that runs ahead of that reading can move it, and nothing that runs after it can.** Ahead of it are stage 0's `reach` rules, which are predicates and have no business moving anybody, and the whole of stage 1. A `world.beforeEachTurn` or `world.before` rule that moves the player — a current carrying the boat at the start of the turn — hands the rest of that turn to the room it carried them into, and the room they typed in gets none of its own rules. A `before` rule at stage 2 or 3, or the default action at stage 4, walks them on without moving it, which is why `go north` runs the `after` rules of the room it *left*. The one way stage 4's walk can move it is ``proceed()`` called from a stage-1 rule, which runs stage 4 inside stage 1: a `world.before(.go)` rule that wraps the default action that way leaves the `after` rules to the room the player walked into.

That reading is what stage 2's ``Location/beforeEachTurn(perform:)`` pass and stage 5's ``Location/after(_:perform:)`` rules are looked up against. Two lookups stand outside it. Stage 2's ``Location/before(_:perform:)`` rules are keyed on the room the **addressee** is standing in when the player told somebody else to act, and on the reading only when nobody was told. And stage 6 takes a fresh reading of its own, so ``Location/afterEachTurn(perform:)`` always belongs to the room the turn *ends* in, however many times the turn moved the player.

A multi-object command takes its own reading for the once-per-turn upkeep it pulls ahead of its object loop, after the world's `beforeEachTurn` rules. Its `before` rules run per object, inside that loop, so they are the one thing the upkeep's reading cannot see: a `world.before` rule that moves the player redirects the location `beforeEachTurn` pass on a single-object command but not on `take all`, where those rules have not run yet when the upkeep goes by.
