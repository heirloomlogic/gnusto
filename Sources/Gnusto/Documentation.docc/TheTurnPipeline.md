...
5. **`after` rules** — ``Item/after(_:perform:)`` rules for the direct object then indirect object, then ``Location/after(_:perform:)`` rules for the current room, then ``World/after(_:perform:)`` rules. An earlier stage that ends the turn skips all three: a `refuse` or `reply` in a `before` rule does, and so does the default action's own refusal.
