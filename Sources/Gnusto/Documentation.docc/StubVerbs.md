...
Deliberately **not** stubs: `notify`/`script` are missing *features*, and a canned refusal would be a worse answer than the error. `again`/`g` and `oops` were on that list until they stopped being missing, and `verbose`, `brief` and `superbrief` — the last of which the list never named — went the same way. The first two act on a line the pipeline has already finished with; the last three set a session preference that lives on the `GameWorld` actor rather than in the world. No stage can answer any of them, so all five are engine-level rows beside UNDO rather than stubs. `ask`/`tell`/`show` belong to `GnustoConversation`, which has a topic slot
