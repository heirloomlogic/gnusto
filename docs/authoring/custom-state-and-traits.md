...
Any `Codable & Sendable` type can be a global. Conform it to `GlobalValue` with an empty conformance — the default implementation JSON-boxes it into the type-erased `StateValue.data` case, so there's nothing to hand-pack:
...
Read and write it in rules exactly like a scalar global — it participates in the turn's commit and in save/restore through the one `WorldState` funnel:
...
> **Save compatibility.** A custom global is stored as opaque bytes, so if you change its shape, an old save may no longer decode — the global then falls back to its declared default. Keep plugin/system state structs additive and optional-tolerant (new fields with defaults), and you'll stay compatible. Versioned codecs are a later effort.

## Custom traits

Declare a custom property inside an `Item { … }` or `Location { … }` block with `trait(key, value)`. The value is boxed with the same rule as a `@Global`, so a scalar or a whole struct both work:
...
Custom traits are **immutable declared facts** — they never touch `WorldState`. For per-entity state that *changes* during play (an item's current charge, a creature's HP), use a `@Global` keyed however your system needs; traits are for the fixed properties an entity is born with.
