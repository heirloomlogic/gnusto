...
| Verbs | Nine intents — `drink`, `rest`, `give`, `talk`, `ring`, `pull`, `sit`, `pet`, `harness` — each with the phrasings a player might actually reach for. Three are the game's own (`ring`, `pet`, `harness`); the other six are the engine's stub verbs, given their extra words in `verbs` and their mechanic in `actions`. | A verb a player cannot guess is a verb the game does not have. The haul answers to seven words because nobody who has not driven a mule knows which one is right. |
