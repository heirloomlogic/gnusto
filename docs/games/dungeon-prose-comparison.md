...
Every room and object whose **display name** appears both in the 1981-07-22
mainframe MDL and in one of Zork I/II/III, with the two descriptions side by
side. This document exists to settle one decision: **when the mainframe text and
the trilogy text differ, which one does `Sources/Dungeon/` use?**
...
- **Identical (36)** — the trilogy carried the mainframe line
  across unchanged. Listed as ids only; there is nothing to decide.
- **Minor (23)** — same sentence, changed punctuation or a
  reworded clause (≥85% similar). Skim.
- **Substantial (48)** — genuinely different writing, or text
...
listed in `dungeon-atlas.md`.

## Substantial differences (48)
...
> `BEGG`, `BKEXE`, `BKVAU`, `BLABE`, `BRICK`, `BUCKE`, `BUOY`, `BWELL`, `CARD`, `CHALI`, `COFFI`, `DBALL`, `DIAMO`, `ENGRA`, `FOOD`, `GNOME`, `HBELL`, `IBOAT`, `ICE`, `IRBOX`, `JADE`, `LEDG3`, `MPEAR`, `PASS1`, `PASS5`, `PEARL`, `RBTLB`, `STAMP`, `STRAD`, `TROLL`, `TSHAF`, `TUBE`, `TWELL`, `VAIR1`, `VAIR2`, `ZGNOM`
