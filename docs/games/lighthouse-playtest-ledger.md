...
**2026-09-08 — the `talk to me` fix superseded; the row stays `fixed`.** `talk` is one of the engine's stub verbs now (#405), so Lighthouse stops minting a verb and reclaims the word instead: the keeper's rule answers her, and every other noun gets the stock line — *You talk for a while. Nobody answers.* A stub costs a turn, as every verb does, so the tide moves under `talk to me` again and the 2026-07-30 amendment above no longer describes shipped behavior. That is not the finding coming back. The round-1 row is keyed `mechanic-contradicts-prose`, and the contradiction was the game insisting it had not understood the sentence *while* spending the turn; the free turn was never the fix, only the shape `TurnInterrupt.unhandled` happens to have for an intent nothing answers. `talk` is answered now, so it costs what it costs. Making the reply free again would hand the player a jetty turn the tide is not charged for. Three warnings and a drowning on the fourth is the story's current shape (`docs/games/lighthouse.md`, "The tide") rather than anything the mechanics contract pins — the contract fixes one always-running daemon with its danger confined to one room, and lists the tide's specific numbers as free to change — but the way to move that deadline is to move the tide, not to exempt one verb from the turn every other verb costs. The deadline therefore stands: on the jetty three `talk to me` warn and the fourth drowns, exactly as four `wait` would. Regression cover is now two tests — `LighthouseTranscriptTests.talkingToNobodyOnTheJettyAnswersInVoiceAndCostsATurn` for the line, the absent parse-failure claim and the turn cost, and `talkingOnTheJettySpendsTheTideDeadlineLikeAnyOtherVerb` for the deadline.
