...
| `unanswerable-noun` (11 rows) | Twelve scenery items in a new `Fixtures` bundle plus three in `Tower`, and synonyms on the existing items for the parts their own descriptions name. | `theJettyAnswersToItsOwnDescription`, `theBaseAndStoreroomAnswerToTheirOwnDescriptions`, `theLampRoomAnswersToItsOwnDescription`, `theKeeperAnswersToHerselfAndHerLeg` |
