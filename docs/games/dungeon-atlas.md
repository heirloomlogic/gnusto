# Dungeon — content atlas

The machine-extracted inventory of mainframe Zork (MDL, 1981-07-22), the
artifact `Sources/Dungeon/` reconstructs. Every figure here comes from the
source, not from a walkthrough or a memory.

<!-- GENERATED by .context/build_atlas.py — do not edit by hand. -->

> **Provenance.** Mainframe figures are extracted from the 1981-07-22 MDL
> `dung.355`; trilogy figures from the Zork I/II/III ZIL sources. Both bodies of
> source are permissively licensed — see `THIRD_PARTY_NOTICES` at the repo root
> for the two separate grants and their limits.
>
> Generated against Gnusto `2391a75`.

## Scoring

The mainframe keeps **two separate maxima**, and the `score` command reports
whichever the player is currently inside (`rooms.394`, `SCORE-BLESS`). They are
not summed anywhere in the source.

| Maximum | Value | Composition |
|---|---:|---|
| `SCORE-MAX` (main dungeon) | **591** | 115 room `RVAL` + 466 object `OFVAL`+`OTVAL` + 10 `LIGHT-SHAFT` |
| `EG-SCORE-MAX` (endgame) | **100** | room `RVAL` over the 31 `RENDGAME` rooms |

Both are accumulated at build time by `makstr.44`: a room's `RVAL` goes to
`EG-SCORE-MAX` if it carries `RENDGAME` and to `SCORE-MAX` otherwise, while
every object's `OFVAL`+`OTVAL` goes to `SCORE-MAX` unconditionally.

`LIGHT-SHAFT` is the one award that lives outside the dungeon file: `act2.92`
pays 10 points, once, for reaching the Lower Shaft while it is lit — the basket
and torch puzzle. In Gnusto terms it is an `awardOnce` register, not a room
`VALUE`.

### What `Sources/Dungeon/` uses

**`maxScore` = 691** — a single ceiling, the sum of both maxima.

The commonly cited "616 points" appears nowhere in this source, so it is not a
candidate. The choice is between the original's two separate maxima (591 and 100,
reported one at a time by `SCORE-BLESS` in `rooms.394`) and one combined figure.
**Decision: one ceiling of 691.**

The reasoning:

- Gnusto models a single `maxScore`, and its bootstrap already totals the award
  table against it and warns on a mismatch. One ceiling makes that invariant do
  real work; two would need the check disabled or worked around.
- 691 is genuinely everything a player can score, so the progress
  reading is honest end to end.
- The cost is small and cosmetic: a player who finishes the main dungeon perfectly
  sees 591/691 where the original showed 591/591,
  and the endgame then carries them the rest of the way rather than restarting at
  zero. That is a divergence, and belongs in `FIDELITY.md`.

## Totals

| | Count |
|---|---:|
| Rooms | 196 |
| — of them endgame (`RENDGAME`) | 31 |
| Objects | 209 |
| — of them valued (treasures) | 31 |

## Rooms

### Main dungeon (156 rooms)

| id | name | RVAL | exits | trilogy | in `Sources/Zork1/` |
|---|---|---:|---:|---|---|
| `ALICE` | Tea Room |  | 3 | Zork II | — |
| `ALISM` | Posts Room |  | 4 | Zork II | — |
| `ALITR` | Pool Room |  | 2 | Zork II | — |
| `ATLAN` | Atlantis Room |  | 2 | Zork I | `Sources/Zork1/Regions/Mirror.swift` |
| `ATTIC` | Attic |  | 1 | Zork I | `Sources/Zork1/House.swift` |
| `BATS` | Bat Room |  | 1 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `BEACH` | Sandy Beach |  | 2 | Zork I | `Sources/Zork1/Regions/River.swift` |
| `BLADD` | Ladder Bottom |  | 3 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `BLROO` | Strange Passage | 10 | 2 | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `BOOM` | Gas Room |  | 1 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `BSHAF` | Lower Shaft |  | 4 | — | — |
| `BWELL` | Circular Room |  | 2 | Zork II | — |
| `CAGED` | Cage |  | 1 | Zork II | — |
| `CAGER` | Dingy Closet |  | 1 | Zork II | — |
| `CANY1` | Deep Canyon |  | 3 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `CAROU` | Round room |  | 9 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `CAVE1` | Cave |  | 2 | — | `Sources/Zork1/Regions/Mirror.swift` |
| `CAVE2` | Cave |  | 3 | — | `Sources/Zork1/Regions/Mirror.swift` |
| `CAVE3` | Damp Cave |  | 3 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `CAVE4` | Engravings Cave |  | 2 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `CELLA` | Cellar | 25 | 4 | Zork I | `Sources/Zork1/House.swift` |
| `CHAS1` | Chasm |  | 3 | Zork I | `Sources/Zork1/Cellar.swift` |
| `CHAS2` | West of Chasm |  | 4 | — | — |
| `CHAS3` | Ancient Chasm |  | 4 | — | — |
| `CLBOT` | Canyon Bottom |  | 2 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `CLEAR` | Clearing |  | 7 | — | `Sources/Zork1/AboveGround.swift` |
| `CLMID` | Rocky Ledge |  | 2 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `CLTOP` | Canyon View |  | 3 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `CMACH` | Machine Room |  | 2 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `CP` | Room in a Puzzle |  | 9 | — | — |
| `CPANT` | Small Square Room |  | 3 | — | — |
| `CPOUT` | Side Room |  | 2 | — | — |
| `CRAW1` | Rocky Crawl |  | 3 | — | — |
| `CRAW2` | Steep Crawlway |  | 2 | — | — |
| `CRAW3` | Narrow Crawlway |  | 3 | — | — |
| `CRAW4` | North-South Crawlway |  | 4 | — | — |
| `CYCLO` | Cyclops Room |  | 3 | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `DAM` | Dam |  | 4 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `DEAD1` | You have come to a dead end in the maze. |  | 1 | — | — |
| `DEAD2` | You have come to a dead end in the maze. |  | 1 | — | — |
| `DEAD3` | Dead End |  | 1 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `DEAD4` | Dead End |  | 1 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `DEAD5` | Dead End |  | 1 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `DEAD6` | Dead End |  | 1 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `DEAD7` | Dead End |  | 1 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `DOCK` | Dam Base |  | 3 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `DOME` | Dome Room |  | 2 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `ECHO` | Loud Room |  | 3 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `EGYPT` | Egyptian Room |  | 3 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `EHOUS` | Behind House |  | 5 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `ENTRA` | Mine Entrance |  | 3 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `FALLS` | Aragain Falls |  | 4 | Zork I | `Sources/Zork1/Regions/River.swift` |
| `FANTE` | Shore |  | 3 | Zork I | `Sources/Zork1/Regions/River.swift` |
| `FCHMP` | Moby lossage |  | 1 | — | — |
| `FORE1` | Forest |  | 5 | — | `Sources/Zork1/AboveGround.swift` |
| `FORE2` | Forest |  | 5 | — | `Sources/Zork1/AboveGround.swift` |
| `FORE3` | Forest |  | 5 | — | `Sources/Zork1/AboveGround.swift` |
| `FORE4` | Forest |  | 5 | — | `Sources/Zork1/AboveGround.swift` |
| `FORE5` | Forest |  | 5 | — | `Sources/Zork1/AboveGround.swift` |
| `GALLE` | Gallery |  | 3 | Zork I | `Sources/Zork1/Cellar.swift` |
| `ICY` | Glacier Room |  | 3 | — | — |
| `INSTR` | Stream |  | 3 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `KITCH` | Kitchen | 10 | 5 | Zork I | `Sources/Zork1/House.swift` |
| `LAVA` | Lava Room |  | 2 | Zork II | — |
| `LEDG2` | Narrow Ledge |  | 4 | Zork II | — |
| `LEDG3` | Volcano View |  | 3 | Zork II | — |
| `LEDG4` | Wide Ledge |  | 4 | Zork II | — |
| `LIBRA` | Library |  | 2 | Zork II | — |
| `LLD1` | Entrance to Hades |  | 3 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `LLD2` | Land of the Living Dead | 30 | 3 | — | — |
| `LOBBY` | Dam Lobby |  | 3 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `LROOM` | Living Room |  | 3 | Zork I | `Sources/Zork1/House.swift` |
| `MACHI` | Machine Room |  | 1 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MAGNE` | Low Room |  | 9 | Zork II | — |
| `MAINT` | Maintenance Room |  | 2 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `MAZ10` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZ11` | Maze |  | 4 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZ12` | Maze |  | 5 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZ13` | Maze |  | 4 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZ14` | Maze |  | 4 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZ15` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE1` | Maze |  | 4 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE2` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE3` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE4` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE5` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE6` | Maze |  | 4 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE7` | Maze |  | 5 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE8` | Maze |  | 3 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MAZE9` | Maze |  | 6 | — | `Sources/Zork1/Regions/Maze.swift` |
| `MGRAI` | Grail Room |  | 3 | — | — |
| `MGRAT` | Grating Room |  | 2 | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `MINE1` | Coal mine |  | 3 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MINE2` | Coal mine |  | 4 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MINE3` | Coal mine |  | 3 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MINE4` | Coal mine |  | 4 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MINE5` | Coal mine |  | 6 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MINE6` | Coal mine |  | 3 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MINE7` | Coal mine |  | 4 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `MIRR1` | Mirror Room |  | 3 | — | `Sources/Zork1/Regions/Mirror.swift` |
| `MIRR2` | Mirror Room |  | 3 | — | `Sources/Zork1/Regions/Mirror.swift` |
| `MPEAR` | Pearl Room |  | 2 | Zork II | — |
| `MTORC` | Torch Room |  | 3 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `MTROL` | The Troll Room |  | 4 | Zork I | — |
| `NHOUS` | North of House |  | 4 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `PALAN` | Dreary Room |  | 3 | Zork II | — |
| `PASS1` | East-West Passage | 5 | 4 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `PASS3` | Cold Passage |  | 3 | Zork I | `Sources/Zork1/Regions/Mirror.swift` |
| `PASS4` | Winding Passage |  | 2 | Zork I | `Sources/Zork1/Regions/Mirror.swift` |
| `PASS5` | North-South Passage |  | 3 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `POG` | End of Rainbow |  | 5 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `PRM` | Tiny Room |  | 4 | Zork II | — |
| `RAINB` | Rainbow Room |  | 2 | — | — |
| `RAVI1` | Deep Ravine |  | 4 | — | — |
| `RCAVE` | Rocky Shore |  | 2 | — | — |
| `RESEN` | Reservoir North |  | 4 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `RESER` | Reservoir |  | 5 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `RESES` | Reservoir South |  | 6 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `RIDDL` | Riddle Room |  | 2 | Zork II | — |
| `RIVR1` | Frigid River |  | 5 | — | `Sources/Zork1/Regions/River.swift` |
| `RIVR2` | Frigid River |  | 3 | — | `Sources/Zork1/Regions/River.swift` |
| `RIVR3` | Frigid River |  | 5 | — | `Sources/Zork1/Regions/River.swift` |
| `RIVR4` | Frigid River |  | 5 | — | `Sources/Zork1/Regions/River.swift` |
| `RIVR5` | Frigid River |  | 3 | — | `Sources/Zork1/Regions/River.swift` |
| `RUBYR` | Ruby Room |  | 2 | — | — |
| `SAFE` | Dusty Room |  | 1 | Zork II | — |
| `SHOUS` | South of House |  | 4 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `SLEDG` | Slide Ledge |  | 3 | — | — |
| `SLID1` | Slide |  | 2 | — | — |
| `SLID2` | Slide |  | 2 | — | — |
| `SLID3` | Slide |  | 3 | — | — |
| `SLIDE` | Slide Room |  | 3 | Zork I | `Sources/Zork1/Regions/Mirror.swift` |
| `SMELL` | Smelly Room |  | 2 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `SPAL` | Sooty Room |  | 1 | — | — |
| `SQUEE` | Squeaky Room |  | 2 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `STREA` | Stream View |  | 3 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `STUDI` | Studio |  | 3 | Zork I | `Sources/Zork1/Cellar.swift` |
| `TCAVE` | Small Cave |  | 2 | — | — |
| `TEMP1` | Temple |  | 2 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `TEMP2` | Altar |  | 1 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `TIMBE` | Timber Room |  | 2 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `TLADD` | Ladder Top |  | 2 | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `TOMB` | Tomb of the Unknown Implementer |  | 3 | — | — |
| `TREAS` | Treasure Room | 25 | 2 | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `TREE` | Up a Tree |  | 2 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `TSHAF` | Shaft Room |  | 3 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `TUNNE` | Wooden Tunnel |  | 3 | — | — |
| `TWELL` | Top of Well | 10 | 2 | Zork II | — |
| `VAIR1` | Volcano Core |  | 0 | Zork II | — |
| `VAIR2` | Volcano near small ledge |  | 2 | Zork II | — |
| `VAIR3` | Volcano near viewing ledge |  | 0 | — | — |
| `VAIR4` | Volcano near wide ledge |  | 2 | Zork II | — |
| `VLBOT` | Volcano Bottom |  | 1 | Zork II | — |
| `WCLF1` | White Cliffs Beach |  | 2 | — | `Sources/Zork1/Regions/River.swift` |
| `WCLF2` | White Cliffs Beach |  | 2 | — | `Sources/Zork1/Regions/River.swift` |
| `WHOUS` | West of House |  | 4 | Zork I | `Sources/Zork1/AboveGround.swift` |

### Bank of Zork (9 rooms)

| id | name | RVAL | exits | trilogy | in `Sources/Zork1/` |
|---|---|---:|---:|---|---|
| `BKBOX` | Safety Depository |  | 4 | Zork II | — |
| `BKENT` | Bank Entrance |  | 3 | Zork II | — |
| `BKEXE` | Chairman's Office |  | 1 | Zork II | — |
| `BKTE` | East Teller's Room |  | 3 | Zork II | — |
| `BKTW` | West Teller's Room |  | 3 | Zork II | — |
| `BKTWI` | Small Room |  | 0 | — | — |
| `BKVAU` | Vault |  | 0 | Zork II | — |
| `BKVE` | Viewing Room |  | 1 | — | — |
| `BKVW` | Viewing Room |  | 1 | — | — |

### Endgame (31 rooms)

| id | name | RVAL | exits | trilogy | in `Sources/Zork1/` |
|---|---|---:|---:|---|---|
| `BDOOR` | Narrow Corridor | 20 | 2 | — | — |
| `CELL` | Prison Cell |  | 3 | — | — |
| `CRYPT` | Crypt | 5 | 2 | Zork II | — |
| `ECORR` | East Corridor |  | 2 | — | — |
| `FDOOR` | Dungeon Entrance | 15 | 5 | — | — |
| `INMIR` | Inside Mirror | 15 | 9 | — | — |
| `MRA` | Hallway |  | 5 | — | — |
| `MRAE` | Narrow Room |  | 4 | — | — |
| `MRANT` | Stone Room |  | 3 | — | — |
| `MRAW` | Narrow Room |  | 4 | — | — |
| `MRB` | Hallway |  | 7 | — | — |
| `MRBE` | Narrow Room |  | 4 | — | — |
| `MRBW` | Narrow Room |  | 4 | — | — |
| `MRC` | Hallway |  | 7 | — | — |
| `MRCE` | Narrow Room |  | 4 | — | — |
| `MRCW` | Narrow Room |  | 4 | — | — |
| `MRD` | Hallway |  | 6 | — | — |
| `MRDE` | Narrow Room |  | 0 | — | — |
| `MRDW` | Narrow Room |  | 0 | — | — |
| `MREYE` | Small Room |  | 4 | — | — |
| `MRG` | Hallway |  | 2 | — | — |
| `MRGE` | Narrow Room |  | 0 | — | — |
| `MRGW` | Narrow Room |  | 0 | — | — |
| `NCELL` | Prison Cell |  | 3 | — | — |
| `NCORR` | North Corridor |  | 5 | — | — |
| `NIRVA` | Treasury of Zork | 35 | 0 | — | — |
| `PARAP` | Parapet |  | 2 | — | — |
| `PCELL` | Prison Cell |  | 1 | — | — |
| `SCORR` | South Corridor |  | 4 | — | — |
| `TSTRS` | Top of Stairs | 10 | 3 | — | — |
| `WCORR` | West Corridor |  | 2 | — | — |

## Objects

`OFVAL` is the score for first acquiring the object, `OTVAL` for depositing it
in the trophy case. `OSIZE` is its weight against the carrying capacity.

| id | name | OSIZE | OFVAL | OTVAL | trilogy | in `Sources/Zork1/` |
|---|---|---:|---:|---:|---|---|
| `ADVER` | leaflet | 2 |  |  | Zork I | `Sources/Zork1/AboveGround.swift` |
| `ARROW` | compass arrow |  |  |  | — | — |
| `ATABL` | large oblong table |  |  |  | Zork II | — |
| `AXE` | bloody axe | 25 |  |  | Zork I | `Sources/Zork1/Cellar.swift` |
| `BAGCO` | bag of coins | 15 | 10 | 5 | — | `Sources/Zork1/Regions/Maze.swift` |
| `BALLO` | basket | 70 |  |  | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `BAR` | platinum bar | 20 | 12 | 10 | Zork I | `Sources/Zork1/Regions/RoundRoom.swift` |
| `BARRE` | wooden barrel | 70 |  |  | — | — |
| `BAT` | bat |  |  |  | Zork I | — |
| `BAUBL` | beautiful brass bauble |  | 1 | 1 | Zork I | — |
| `BCANA` | broken clockwork canary |  |  |  | Zork I | `Sources/Zork1/House.swift` |
| `BEGG` | broken jewel-encrusted egg |  |  |  | Zork I | — |
| `BELL` | bell |  |  |  | — | — |
| `BILLS` | stack of zorkmid bills | 10 | 10 | 15 | Zork II | — |
| `BLABE` | blue label | 1 |  |  | Zork II | — |
| `BLAMP` | broken lamp |  |  |  | — | — |
| `BLANT` | burned-out lantern | 20 |  |  | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `BLBK` | blue book | 10 |  |  | Zork II | — |
| `BLBUT` | blue button |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `BLICE` | piece of cake with blue icing | 4 |  |  | — | — |
| `BLWAL` | black panel |  |  |  | — | — |
| `BODIE` | pile of bodies |  |  |  | Zork I | — |
| `BOLT` | bolt |  |  |  | Zork I | — |
| `BONES` | skeleton |  |  |  | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `BOOK` | book | 10 |  |  | — | — |
| `BOTTL` | glass bottle |  |  |  | Zork I | `Sources/Zork1/House.swift` |
| `BRACE` | sapphire bracelet | 10 | 5 | 3 | — | — |
| `BRBUT` | brown button |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `BRICK` | brick | 9 |  |  | Zork II | — |
| `BROCH` | free brochure | 30 |  |  | — | — |
| `BROPE` | braided wire |  |  |  | Zork II | — |
| `BUBBL` | green bubble |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `BUCKE` | wooden bucket | 100 |  |  | Zork II | — |
| `BUOY` | red buoy | 10 |  |  | Zork I | `Sources/Zork1/Regions/River.swift` |
| `BUTTO` |  |  |  |  | — | — |
| `CAGE` | steel cage | 60 |  |  | — | — |
| `CANDL` | pair of candles | 10 |  |  | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `CARD` | card | 1 |  |  | Zork II | — |
| `CBAG` | cloth bag |  |  |  | Zork II | — |
| `CCLIF` | cliff |  |  |  | — | — |
| `CDOOR` | cell door |  |  |  | — | — |
| `CHALI` | chalice | 10 | 10 | 10 | Zork I | — |
| `COAL` | small pile of coal | 20 |  |  | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `COFFI` | gold coffin | 55 | 3 | 7 | Zork I | `Sources/Zork1/Regions/Temple.swift` |
| `COIN` | priceless zorkmid | 10 | 10 | 12 | Zork II | — |
| `COKES` | bunch of Coke bottles | 15 |  |  | — | — |
| `CORPS` | pile of corpses |  |  |  | — | — |
| `CPANL` | control panel |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `CPDR2` | steel door |  |  |  | — | — |
| `CROWN` | crown | 10 | 15 | 10 | — | — |
| `CYCLO` | cyclops |  |  |  | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `DAM` | dam |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `DBALL` | broken balloon | 40 |  |  | Zork II | — |
| `DBOAT` | plastic boat (with hole) | 20 |  |  | — | — |
| `DBUTT` | large button |  |  |  | — | — |
| `DIAL` | sundial |  |  |  | — | — |
| `DIAMO` | huge diamond |  | 10 | 6 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `DOOR` | trap door |  |  |  | Zork I | `Sources/Zork1/House.swift` |
| `DSTMP` | Don Woods stamp | 1 |  | 1 | — | — |
| `ECAKE` | piece of 'Eat-Me' cake | 10 |  |  | — | — |
| `EGG` | jewel-encrusted egg |  | 5 | 5 | Zork I | `Sources/Zork1/AboveGround.swift` |
| `EIGHT` | number eight |  |  |  | — | — |
| `EMERA` | large emerald |  | 5 | 10 | Zork I | `Sources/Zork1/Regions/River.swift` |
| `ENGRA` | wall with engravings |  |  |  | Zork I | — |
| `ETCH1` | wall with etchings |  |  |  | — | — |
| `ETCH2` | wall with etchings |  |  |  | — | — |
| `FBASK` | lowered basket |  |  |  | — | — |
| `FDOOR` | door |  |  |  | Zork I | — |
| `FIVE` | number five |  |  |  | — | — |
| `FLASK` | glass flask filled with liquid | 10 |  |  | — | — |
| `FOOD` | lunch |  |  |  | Zork I | `Sources/Zork1/House.swift` |
| `FOUR` | number four |  |  |  | — | — |
| `FTREE` | large tree |  |  |  | — | `Sources/Zork1/AboveGround.swift` |
| `FUSE` | wire coil | 1 |  |  | — | — |
| `GARLI` | clove of garlic |  |  |  | Zork I | `Sources/Zork1/House.swift` |
| `GATES` | gates |  |  |  | — | — |
| `GCANA` | clockwork canary |  | 6 | 2 | — | — |
| `GHOST` | number of ghosts |  |  |  | Zork I | — |
| `GNOME` | Volcano Gnome |  |  |  | Zork II | — |
| `GRAIL` | grail | 10 | 2 | 5 | — | — |
| `GRATE` | grating |  |  |  | Zork I | — |
| `GRBK` | green book | 10 |  |  | Zork II | — |
| `GUANO` | hunk of bat guano | 20 |  |  | — | — |
| `GUIDE` | tour guidebook |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `GUNK` | piece of vitreous slag | 10 |  |  | — | — |
| `HBELL` | red hot brass bell |  |  |  | Zork I | — |
| `HEADS` | set of poled heads |  |  |  | Zork II | — |
| `HOOK1` | hook |  |  |  | — | — |
| `HOOK2` | hook |  |  |  | — | — |
| `HPOLE` | head on a pole |  |  |  | — | — |
| `IBOAT` | pile of plastic | 20 |  |  | Zork I | `Sources/Zork1/Regions/River.swift` |
| `ICE` | glacier |  |  |  | Zork II | — |
| `IRBOX` | steel box | 40 |  |  | Zork II | — |
| `JADE` | jade figurine | 10 | 5 | 5 | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `KEYS` | set of skeleton keys | 10 |  |  | — | — |
| `KNIFE` | knife |  |  |  | — | — |
| `LABEL` | tan label | 2 |  |  | Zork I | — |
| `LAMP` | lamp | 15 |  |  | — | — |
| `LCASE` | large case |  |  |  | — | — |
| `LDOOR` | locked door |  |  |  | — | — |
| `LEAK` | leak |  |  |  | — | — |
| `LEAVE` | pile of leaves | 25 |  |  | Zork I | `Sources/Zork1/AboveGround.swift` |
| `LISTS` | stack of listings | 70 |  |  | — | — |
| `LPOLE` | long pole |  |  |  | — | — |
| `MACHI` | machine |  |  |  | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `MAILB` | mailbox |  |  |  | — | — |
| `MAT` | welcome mat | 12 |  |  | — | — |
| `MATCH` | matchbook | 2 |  |  | — | `Sources/Zork1/Regions/Dam.swift` |
| `MDOOR` | locked door |  |  |  | — | — |
| `MSWIT` | switch |  |  |  | Zork I | `Sources/Zork1/Regions/CoalMine.swift` |
| `NEST` | birds nest |  |  |  | — | — |
| `OAKND` | mahogany wall |  |  |  | — | — |
| `ODOOR` | bronze door |  |  |  | — | — |
| `ONE` | number one |  |  |  | — | — |
| `ORICE` | piece of cake with orange icing | 4 |  |  | — | — |
| `OTIMB` | broken timber | 50 |  |  | — | — |
| `PAINT` | painting | 15 | 4 | 7 | Zork I | `Sources/Zork1/Cellar.swift` |
| `PAL3` | red crystal sphere |  | 10 | 5 | Zork II | — |
| `PALAN` | blue crystal sphere |  | 10 | 5 | Zork II | — |
| `PAPER` | newspaper | 2 |  |  | Zork II | — |
| `PCRAK` | narrow crack |  |  |  | Zork II | — |
| `PDOOR` | door made of oak |  |  |  | Zork II | — |
| `PEARL` | pearl necklace | 10 | 9 | 5 | Zork II | — |
| `PINND` | pine wall |  |  |  | — | — |
| `PKEY` | rusty iron key |  |  |  | Zork II | — |
| `PKH1` | keyhole |  |  |  | — | — |
| `PKH2` | keyhole |  |  |  | — | — |
| `PLEAK` | leak |  |  |  | — | — |
| `PLID1` | metal lid |  |  |  | — | — |
| `PLID2` | metal lid |  |  |  | — | — |
| `POOL` | pool of sewage |  |  |  | — | — |
| `PORTR` | portrait of J. Pierpont Flathead | 25 | 10 | 5 | Zork II | — |
| `POSTS` | group of wooden posts |  |  |  | Zork II | — |
| `POT` | pot of gold | 15 | 10 | 10 | Zork I | `Sources/Zork1/Regions/River.swift` |
| `PRAYE` | prayer |  |  |  | Zork I | — |
| `PTABL` | table |  |  |  | — | — |
| `PUBK` | purple book | 10 |  |  | Zork II | — |
| `PUMP` | hand-held air pump |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `PUTTY` | viscous material | 6 |  |  | Zork I | — |
| `PWIND` | barred window |  |  |  | Zork II | — |
| `QDOOR` | wooden door |  |  |  | — | — |
| `RAILI` | wooden railing |  |  |  | Zork I | — |
| `RAINB` | rainbow |  |  |  | Zork I | — |
| `RBEAM` | red beam of light |  |  |  | — | — |
| `RBOAT` | magic boat | 20 |  |  | Zork I | `Sources/Zork1/Regions/River.swift` |
| `RBTLB` | green piece of paper | 3 |  |  | Zork II | — |
| `RBUTT` | red button |  |  |  | — | `Sources/Zork1/Regions/Dam.swift` |
| `RCAGE` | steel cage |  |  |  | — | — |
| `RDICE` | piece of cake with red icing | 4 |  |  | — | — |
| `RDWAL` | red panel |  |  |  | — | — |
| `RECEP` | receptacle |  |  |  | Zork II | — |
| `REFL1` | mirror |  |  |  | — | `Sources/Zork1/Regions/Mirror.swift` |
| `REFL2` | mirror |  |  |  | — | `Sources/Zork1/Regions/Mirror.swift` |
| `RKNIF` | rusty knife | 20 |  |  | Zork I | `Sources/Zork1/Regions/Maze.swift` |
| `RNBUT` | round button |  |  |  | Zork II | — |
| `ROBOT` | robot |  |  |  | — | — |
| `ROPE` | rope | 10 |  |  | — | — |
| `RSWIT` | red button |  |  |  | — | `Sources/Zork1/Regions/Dam.swift` |
| `RUBY` | ruby |  | 15 | 8 | Zork II | — |
| `RUG` | carpet |  |  |  | Zork I | — |
| `SAFE` | box |  |  |  | Zork II | — |
| `SAFFR` | tin of spices | 8 | 5 | 5 | — | — |
| `SAND` | sandy beach |  |  |  | — | `Sources/Zork1/Regions/River.swift` |
| `SBAG` | brown sack | 3 |  |  | Zork I | `Sources/Zork1/House.swift` |
| `SCOL` | shimmering curtain of light |  |  |  | Zork II | — |
| `SCREW` | screwdriver |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `SDOOR` | stone door |  |  |  | — | — |
| `SEVEN` | number seven |  |  |  | — | — |
| `SHOVE` | shovel | 15 |  |  | Zork I | `Sources/Zork1/Regions/River.swift` |
| `SIX` | number six |  |  |  | — | — |
| `SPHER` | white crystal sphere | 10 | 6 | 6 | — | — |
| `SPOLE` | short pole |  |  |  | — | — |
| `SQBUT` | square button |  |  |  | Zork II | — |
| `SSLOT` | hole |  |  |  | — | — |
| `STAMP` | Flathead stamp | 1 | 4 | 10 | Zork II | — |
| `STATU` | statue | 8 | 10 | 13 | — | — |
| `STICK` | broken sharp stick | 3 |  |  | — | — |
| `STILL` | stiletto | 10 |  |  | Zork I | `Sources/Zork1/Thief.swift` |
| `STOVE` | old coal stove |  |  |  | — | — |
| `STRAD` | fancy violin | 10 | 10 | 10 | Zork II | — |
| `SWORD` | sword | 30 |  |  | — | — |
| `TBAR` | T-bar |  |  |  | — | — |
| `TBASK` | basket |  |  |  | — | `Sources/Zork1/Regions/CoalMine.swift` |
| `TCASE` | trophy case |  |  |  | Zork I | `Sources/Zork1/House.swift` |
| `TCHST` | group of tool chests |  |  |  | Zork I | — |
| `THIEF` | thief |  |  |  | Zork I | `Sources/Zork1/Thief.swift` |
| `THREE` | number three |  |  |  | — | — |
| `TOMB` | crypt door |  |  |  | Zork II | — |
| `TORCH` | torch | 20 | 14 | 6 | — | — |
| `TRBUT` | triangular button |  |  |  | Zork II | — |
| `TRIDE` | crystal trident | 20 | 4 | 11 | Zork I | `Sources/Zork1/Regions/Mirror.swift` |
| `TROLL` | troll |  |  |  | Zork I | `Sources/Zork1/Cellar.swift` |
| `TRUNK` | trunk of jewels | 35 | 15 | 8 | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `TTREE` | large tree |  |  |  | — | `Sources/Zork1/AboveGround.swift` |
| `TUBE` | tube | 10 |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `TWO` | number two |  |  |  | — | — |
| `VAULT` | large stone cube |  |  |  | Zork II | — |
| `WARNI` | note of warning |  |  |  | — | — |
| `WATER` | quantity of water | 4 |  |  | — | `Sources/Zork1/House.swift` |
| `WCLIF` | white cliffs |  |  |  | Zork I | — |
| `WDBAR` | wooden bar |  |  |  | — | — |
| `WDOOR` | wooden door |  |  |  | — | — |
| `WHBK` | white book | 10 |  |  | Zork II | — |
| `WHWAL` | white panel |  |  |  | — | — |
| `WINDO` | window |  |  |  | — | — |
| `WRENC` | wrench | 10 |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `YBUTT` | yellow button |  |  |  | Zork I | `Sources/Zork1/Regions/Dam.swift` |
| `YLWAL` | yellow panel |  |  |  | — | — |
| `ZGNOM` | Gnome of Zurich |  |  |  | Zork II | — |

## Coverage

| | Rooms | Objects |
|---|---:|---:|
| Total in the 1981 MDL | 196 | 209 |
| Matched to a trilogy entity by name | 85 | 106 |
| Already built in `Sources/Zork1/` | 104 | — |
| Display name too ambiguous to match | 70 | 142 |

Name matching is exact on the display name, so this **understates** the true
overlap — the trilogy renamed a number of rooms it kept. Treat these as floors,
not as final figures.

The last row is the maze problem. Fifteen mainframe passages share the name
*Maze*, nine share *Dead End*, and the trilogy does the same — so the name
identifies no single room and any pairing built on it would be arbitrary. Those
entities are excluded from matching rather than matched wrongly; pairing them
needs the exit graph, which is future work.

## Open questions

1. **Objects declared but never placed.** Several valued objects appear in no
   room's contents list because they start inside another object (the egg in the
   nest, the emerald in the buoy) or are created by a puzzle (the diamond). The
   atlas does not yet distinguish these from genuinely unreachable content.

